...
function Header({ OpenSidebar }) {
  ...
  const fetchAttendanceNotifications = async () => {
    try {
      ...
      if (response.data.success) {
        const notifications = response.data.data.map(notif => {
          return {
            id: `attendance-${notif.AttendanceId || notif.id}`,
    ...
    try {
      ...
      if (response.data.success) {
        const notifications = response.data.data.map(notif => {
          ...
          // Create a title based on pay structure
          let title = '';
          let type = 'alert';
          ...
          return {
            id: `variable-pay-${notif.VariablePayId}`,
            type: type,
            title: title,
        ...
        console.log('Variable Pay notifications:', notifications);
    ...
    try {
      ...
      if (response.data.success) {
        const notifications = response.data.data.map(notif => {
          ...
          // Include per hour wages if available
          ...
          return {
            id: `wages-approval-${notif.ApprovalID}`,
            type: 'wagesApproval',
        ...
        console.log('Wages Approval notifications:', notifications);
  ...
  const generateAttendanceMessage = (notif) => {
    ...
    return `${labourId}: ${timeInfo} ${remarkInfo}`;
  };
  ...
  useEffect(() => {
    ...
    setUnreadCount(siteTransferUnread + attendanceUnread + alertUnread + updateUnread + otherUnread);
    ...
    return (
      <header className='header' style={{ position: 'relative' }}>
        ...
        >
          ...
          >
            <Typography variant='h6' sx={{ fontWeight: 600, color: '#333' }}>Notifications</Typography>
            {unreadCount > 0 && (
              <Badge badgeContent={unreadCount} color='error'>
                <Typography variant='body2' sx={{ color: '#666' }}>New</Typography>
              </Badge>
            )}
          </Box>
          <Tabs
            ...
            <Tab
              label={
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  {alertNotifications.filter(n => !n.isRead).length > 0 && (
                    <Badge
                      badgeContent={alertNotifications.filter(n => !n.isRead).length}
                      color="error"
                      sx={{ ml: 1 }}
                    />
                  )}
            ...
            <Tab
              label={
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  {updateNotifications.filter(n => !n.isRead).length > 0 && (
                    <Badge
                      badgeContent={updateNotifications.filter(n => !n.isRead).length}
                      color="error"
                      sx={{ ml: 1 }}
                    />
                  )}
