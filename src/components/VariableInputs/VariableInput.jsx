...
const VariableInput = ({ departments, projectNames = [], labour }) => {
    ...
    const confirmTransfer = async () => {
        ...
        try {
            ...
            console.log(`Current Site Name: ${projectNames}, Transfer Site Name:`);
            ...
            const transferDataPayload = {
                ...
                payStructure: selectedLabour.payStructure,
                effectiveDate: selectedLabour.effectiveDate,
            ...
            if (response.status === 200) {
                ...
                toast.success(`Labour ${selectedLabour.name} variable pay Add successfully!`);
            } else {
    ...
    const handlePayStructureChange = (e, labourID) => {
        const updatedLabours = labours.map(labour => {
            if (labour.LabourID === labourID) {
                return { ...labour, payStructure: e.target.value };
            }
            return labour;
        });
                                ...
                                <TableRow key={labour.LabourID}>
                                    ...
                                    <TableCell>
                                        <TextField
                                            ...
                                            onChange={(e) =>
                                                handleVariablePayChange(e, labour.LabourID)
                                            }
                                            sx={{ mb: 2 }}
                                        />
                                    </TableCell>
                                    <TableCell>
                                        <TextField
                                            ...
                                            inputProps={{
                                                min: new Date(new Date().setDate(new Date().getDate() - 2)).toISOString().split('T')[0], // Minimum date set to two days before today
                                                max: new Date(new Date().setDate(new Date().getDate() + 2)).toISOString().split('T')[0], // Maximum date set to two days after today
                                            }}
                                            sx={{ mb: 2 }}
                                            required
                                        />
