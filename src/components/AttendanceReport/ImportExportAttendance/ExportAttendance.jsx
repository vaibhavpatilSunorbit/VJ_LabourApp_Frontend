...
import axios from 'axios';
import {
    Button, Box, TextField, Select, MenuItem, Typography, Modal, Grid, FormControl, RadioGroup, FormControlLabel, Radio
} from '@mui/material';
import { API_BASE_URL } from "../../../Data";
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
...
const ExportAttendance = () => {
    ...
    const handleExport = async () => {
        ...
        try {
            // Use different endpoints based on export format
            ...
            const fileName = `Attendance_${startDate}_${endDate}.${fileExtension}`;
            ...
            toast.success(`Attendance exported as ${exportFormat.toUpperCase()} successfully!`);
        } catch (error) {
            console.error('Error exporting data:', error);
            if (error.response && error.response.data && error.response.data.message) {
                toast.error(`Export Error: ${error.response.data.message}`);
                        ...
                        <Grid container spacing={2} justifyContent="flex-end">
                            ...
                            <Grid item>
                                <Button
                                    onClick={handleExport}
                                    variant="contained"
                                    startIcon={exportFormat === 'pdf' ? <PictureAsPdfIcon /> : <FileDownloadOutlinedIcon />}
                                    sx={{
                                        backgroundColor: exportFormat === 'pdf' ? '#d32f2f' : '#4CAF50',
                                        color: '#fff',
                                        '&:hover': {
                                            backgroundColor: exportFormat === 'pdf' ? '#c62828' : '#45a049',
                                        },
                                    }}
                                >
