...
const UserApproval = ({ onFormSubmit }) => {
  ...
  const [pendingAttendanceUser, setPendingAttendanceUser] = useState(0);
  const [approvedAttendanceUser, setApprovedAttendanceUser] = useState(0);
  const [rejectedAttendanceUser, setRejectedAttendanceUser] = useState(0);
  ...
  const [error, setError] = useState('');
   ...
   const fetchAttendance = async () => {
    try {
      ...
      const encData = response.data.filter(labour => labour.userType === 'ENC');
      const pending = encData.data.filter(labour => labour.ApprovalStatus === "Pending").length;
      const approved = encData.data.filter(labour => labour.ApprovalStatus === "Approved").length;
      const rejected = encData.data.filter(labour => labour.ApprovalStatus === "Rejected").length;
      setPendingAttendanceUser(pending);
      setApprovedAttendanceUser(approved);
      setRejectedAttendanceUser(rejected);
    } catch (err) {
  ...
  return (
    <Grid container spacing={8} sx={{ mt: 0, px: 8 }}>
      <Grid item xs={12} sm={6} md={4}>
        <Link to={'/userApproval/UserAttedanceApproval'} style={{ textDecoration: 'none' }}>
          <Card
            sx={{
          ...
          >
            <CardContent sx={{ textAlign: 'center' }}>
              ...
              >
                <Typography>Pending: {pendingAttendanceUser}</Typography>
                <Typography>Approved: {approvedAttendanceUser}</Typography>
                <Typography>Rejected: {rejectedAttendanceUser}</Typography>
              </Box>
            </CardContent>
          </Card>
        </Link>
