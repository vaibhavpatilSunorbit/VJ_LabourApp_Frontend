...
const UserAttedanceApproval = ({ departments, projectNames, labour, labourlist }) => {
    const { user } = useUser();
    ...
    const fetchLabours = async () => {
        ...
        try {
            ...
            const encLabours = response.data.filter(labour => labour.userType === 'ENC');
            setLabours(encLabours);
            //   setLabours(response.data.map(labour => ({
            //     ...labour
            //   })));
            const pendingAttendanceUser = response.data.filter((labour) => labour.ApprovalStatus === "Pending"&& labour.userType === "ENC").length;
            const approvedAttendanceUser = response.data.filter((labour) => labour.ApprovalStatus === "Approved"&& labour.userType === "ENC").length;
            const rejectedAttendanceUser = response.data.filter((labour) => labour.ApprovalStatus === "Rejected"&& labour.userType === "ENC").length;

            setPendingCount(pendingAttendanceUser);
            setApprovedCount(approvedAttendanceUser);
            setRejectedCount(rejectedAttendanceUser);
            localStorage.setItem('pendingAttendanceUser', pendingAttendanceUser);
            localStorage.setItem('approvedAttendanceUser', approvedAttendanceUser);
            localStorage.setItem('rejectedAttendanceUser', rejectedAttendanceUser);
            setLoading(false);
        } catch (error) {
...
export default UserAttedanceApproval;